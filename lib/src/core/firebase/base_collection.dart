...
class BaseCollectionReference<T> {
  ...
  Future<DataState<T>> get(String id) async {
    try {
      final DocumentSnapshot<T> doc = await ref.doc(id).get();
      if (doc.exists) {
        return DataSuccess(doc.data() as T);
      } else {
        return const DataFailed('Document does not exist');
      }
    } catch (e) {
  ...
  Future<DataState<T>> add(T item) async {
    try {
      final DocumentReference<T> doc =
          await ref.add(item).timeout(const Duration(seconds: 5));
      return DataSuccess(setObjectId(item, doc.id));
    } catch (e) {
      return DataFailed.exception(e);
    }
  }

  Future<DataState<T>> set(T item, {bool merge = true}) async {
    try {
      await ref
  ...
  Future<DataState> update(String? id, Map<Object, Object?> data) async {
    if (id == null) {
  ...
  Future<DataState<String>> delete(String id) async {
    try {
  ...
  Future<DataState<List<T>>> getAll() async {
    try {
  ...
  Future<DataState<List<T>>> getDataByIds(List<String> rooms) async {
    if (rooms.isEmpty) {
      return const DataSuccess([]);
    }
    try {
