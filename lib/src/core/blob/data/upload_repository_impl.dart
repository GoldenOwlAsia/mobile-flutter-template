import 'package:http_parser/http_parser.dart' as http_parser;
import 'package:myapp/src/core/resources/data_state.dart';
import '../model/upload_model.dart';
import 'upload_repository.dart';

class UploadRepositoryImpl extends UploadRepository {
  // Submit content must be image, video or audio
  Future<DataState<MUpload>> uploadFile(
    String path,
    http_parser.MediaType mediaType,
  ) async {
    return const DataFailed('Not implement api upload yet!');
  }

  @override
  Future<DataState<MUpload>> uploadImage(String path) async {
    return uploadFile(path, http_parser.MediaType('image', 'png'));
  }

  @override
  Future<DataState<MUpload>> uploadVideo(String path) async {
    return uploadFile(path, http_parser.MediaType('video', 'mp4'));
  }

  @override
  Future<List<DataState<MUpload>>> uploadImages(List<String> paths) {
    return Future.wait([for (final e in paths) uploadImage(e)]);
  }

  @override
  Future<List<DataState<MUpload>>> uploadVideos(List<String> paths) {
    return Future.wait([for (final e in paths) uploadVideo(e)]);
  }
}
